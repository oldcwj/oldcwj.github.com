...
function nav(active, depth = 0) {
  ...
  if (active === "home") {
    return `<header class="site-header home-header"><div class="nav"><a class="brand" href="${prefix}index.html"><img class="site-logo" src="${prefix}assets/icons/coobbi.png" alt="Coobbi logo"><span>Coobbi</span></a><div class="nav-links"><a class="active" href="#web-tools">Tools</a><a href="#mobile-apps">Mobile Apps</a><a href="#guides">Guides</a><a href="${prefix}contact.html">About</a><a class="nav-cta" href="${prefix}file-inspector/index.html">Open File Inspector</a></div></div></header>`;
  }
  const items = [
  ...
  return `<header class="site-header"><div class="nav"><a class="brand" href="${prefix}index.html"><img class="site-logo" src="${prefix}assets/icons/coobbi.png" alt="Coobbi logo"><span>Coobbi</span></a><div class="nav-links">${items.map(([label, href, key]) => `<a ${active === key ? 'class="active"' : ""} href="${prefix}${href}">${label}</a>`).join("")}</div></div></header>`;
}
...
function head({ title, description, canonical, image, depth = 0, structuredData = [] }) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><link rel="icon" type="image/png" href="${prefix}assets/icons/coobbi.png"><meta name="description" content="${esc(description)}"><meta name="google-adsense-account" content="${adsenseAccount}"><link rel="canonical" href="${canonical}"><link rel="stylesheet" href="${prefix}assets/style.css">
...
function homePage() {
  ...
  return `${head({ title: "Coobbi File Tools and Mobile Apps", description: "Inspect files in your browser and discover Coobbi apps for iPhone, iPad and Android.", canonical: `${appsData.site.url}/`, image: "assets/icons/coobbi.png", structuredData })}<body>${nav("home")}
